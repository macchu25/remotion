...
// Google Translate TTS fetcher for high-quality Vietnamese AI narration
...
// Master V9 Continuous Narrative Vietnamese Subtitle Cues for Chapters 1 - 9
const cues = [
...
async function run() {
  const outDir = path.join(__dirname, 'public', 'audio', 'voice_evolution');
  fs.mkdirSync(outDir, { recursive: true });

  console.log(`Generating Vietnamese Voiceover for Evolution Film (${cues.length} cues)...`);

  for (let i = 0; i < cues.length; i++) {
    const cue = cues[i];
    const targetFile = path.join(outDir, `${cue.id}.mp3`);
    console.log(`[${i + 1}/${cues.length}] Generating: ${cue.id} -> "${cue.text.slice(0, 35)}..."`);

    try {
      const buffer = await fetchTTSChunk(cue.text);
      fs.writeFileSync(targetFile, buffer);
      await new Promise((r) => setTimeout(r, 120));
    } catch (err) {
      console.error(`Failed on ${cue.id}:`, err.message);
    }
  }

  console.log(`\n🎉 Generated all ${cues.length} Vietnamese Voiceover MP3s in ${outDir}!`);
}

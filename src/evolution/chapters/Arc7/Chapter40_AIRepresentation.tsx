...
export const Chapter40_AIRepresentation: React.FC = () => {
  ...
  const spaceScale = spring({ frame: frame - 10, fps, config: { damping: 14, stiffness: 80 } });
  const worldScale = spring({ frame: frame - 330, fps, config: { damping: 14, stiffness: 80 } });
    ...
    <div style={{ position: "relative", width: 1920, height: 1080, overflow: "hidden", backgroundColor: "#020617" }}>
      ...
      <div
        style={{
          ...
          inset: 0,
          opacity: 0.18,
          transform: `scale(${interpolate(frame, [0, 604], [1.0, 1.08], { extrapolateRight: "clamp" })})`,
      ...
      <CameraRig panBehavior="slow_push">
        ...
        >
          <div style={{ color: "#38BDF8", fontSize: 20, letterSpacing: 4, fontWeight: 700 }}>
            CHƯƠNG 40 · BIỂU DIỄN TRI THỨC & KHÔNG GIAN TIỀM ẨN
          </div>
          <div style={{ color: "#FFFFFF", fontSize: 32, fontWeight: 800, marginTop: 8 }}>
            Ngôn Ngữ & Vạn Vật Hóa Thành Vector Đa Chiều
          </div>
        </div>

        {/* Phase 1: Semantic Vector Embeddings */}
        {p1Opacity > 0 && (
          <div style={{ position: "absolute", inset: 0, opacity: p1Opacity }}>
            <div
              style={{
                ...
                left: 960,
                top: 480,
                transform: `translate(-50%, -50%) scale(${spaceScale})`,
                width: "max-content",
                maxWidth: 1550,
                textAlign: "center",
            ...
            >
              <div style={{ color: "#38BDF8", fontSize: 20, letterSpacing: 3, fontWeight: 700, marginBottom: 16 }}>
                KHÔNG GIAN NHÚNG VECTOR NGỮ NGHĨA (EMBEDDINGS)
              </div>
              <KaTeXLabel
                latex="\vec{v}(\text{King}) - \vec{v}(\text{Man}) + \vec{v}(\text{Woman}) \approx \vec{v}(\text{Queen}) \in \mathbb{R}^d"
                isInline={true}
                fontSize={34}
                color="#F8FAFC"
                scaleIn={false}
              ...
              <div style={{ color: "#94A3B8", fontSize: 22, marginTop: 18 }}>
                Từ ngữ, hình ảnh và âm thanh được nhúng thành các vector trong không gian tiềm ẩn hàng nghìn chiều
              </div>
            </div>
          </div>
        )}
          ...
          <div style={{ position: "absolute", inset: 0, opacity: p2Opacity }}>
            ...
            >
              <div style={{ color: "#A855F7", fontSize: 20, letterSpacing: 3, fontWeight: 700, marginBottom: 16 }}>
                CƠ CHẾ CHÚ Ý TỰ THÂN (SELF-ATTENTION)
              </div>
              <KaTeXLabel
              ...
              />
              <div style={{ color: "#94A3B8", fontSize: 22, marginTop: 18 }}>
                Cơ chế chú ý Attention kết nối ngữ cảnh của vạn vật, biến các phép tính ma trận thành sự thấu hiểu sâu sắc
              </div>
            </div>
          </div>
        )}

        {/* Phase 3: Bridge to Modern Civilization */}
        {p3Opacity > 0 && (
          <div style={{ position: "absolute", inset: 0, opacity: p3Opacity }}>
            ...
            >
              <div style={{ color: "#F59E0B", fontSize: 22, letterSpacing: 3, fontWeight: 700, marginBottom: 16 }}>
                HẠT GIỐNG TIẾN HÓA: TOÁN HỌC DƯỚI NỀN VĂN MINH
              </div>
              <KaTeXLabel
                latex="\text{Thế giới Hiện đại} = \text{GPS} + \text{MRI} + \text{Internet} + \text{Thời tiết} + \text{AI}"
                isInline={true}
                fontSize={30}
                color="#F59E0B"
                scaleIn={false}
              />
              <div style={{ color: "#94A3B8", fontSize: 22, marginTop: 18 }}>
                Và trí tuệ nhân tạo hôm nay chỉ là một trong vô vàn nơi toán học đang âm thầm vận hành thế giới
              </div>
            </div>
          </div>
        )}
